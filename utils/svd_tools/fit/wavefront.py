...
class Mesh(object) :
	...
	def _compute_neighbours(self):
		...
		self._neighbours = [set() for i in range(nverts)]
	...
	def edges(self):
		...
		edges = [(x_y[0],x_y[1]) if x_y[0]<=x_y[1] else (x_y[1],x_y[0]) for x_y in edges]
		...
		for line in f.readlines() :
			...
			if   line.startswith('v '): # New vertex
				...
				if mtllib is not None :
					...
					mtllib = None
				object['vertices'].append(list(map(float,line.split()[1:])))
				
			elif line.startswith('vt '):  # texture vertex
				object['uvcoords'].append(list(map(float,line.split()[1:])))
				
			elif line.startswith('vn '):  # vertex normal
				object['normals'].append(list(map(float,line.split()[1:])))
				
			elif line.startswith('f '):   # face
				indices = line.split()[1:]
				if len(indices) < 3 : continue
				if line.find('//')!=-1 : # we also have uv faces information
					coords = [ [int(x) -1 for x in i.split('//')] for i in indices ]
					...
					object['faces'].append( [int(x) -1 for x in line.split()[1:]] )
