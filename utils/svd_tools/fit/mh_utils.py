...
def lookup(vertices_list):
	return dict([(vertices_list[i],i) for i in range(len(vertices))])

def read_target(target,vertices_list = None, look_up = None ):
	if vertices_list is None :
		nvert = NVERTICES
		vertices = list(range(nvert))
		if look_up is None : look_up = list(range(nvert))
	else :
	...
	with open(target) as f :
		for line in f.readlines():
			values = line.split()
			ivert = int(values[0])
			t[look_up[ivert]] = np.array(list(map(float,values[1:])),'float32')
	return t
...
def load_targets(target_files,vertices_list=None):
	...
	if vertices_list is None :
		nvert = NVERTICES
		vertices_list = list(range(NVERTICES))
		look_up = list(range(nvert))
	else :
		nvert = len(vertices_list)
