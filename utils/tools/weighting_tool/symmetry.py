...
def setupVertexPairs(context):
    ...
    print(("Left-right-mid", len(list(lverts.keys())), len(list(rverts.keys())), len(list(mverts.keys()))))
...
def symmetrizeWeights(context, left2right):
    ...
    for (vn, rvn) in list(rverts.items()):
...
def printGroups(name, groups, indices, vgroups):
    print(name)
    for (nameStripped, grp) in list(groups.items()):
