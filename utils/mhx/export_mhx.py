...
def initLocalData():
    global createdLocal
    for key in list(createdLocal.keys()):
        createdLocal[key] = []
...
def exportMTex(index, mtex, use, fp):
    ...
    mapto = None
    prio = []
    for ext in list(MapToTypes.keys()):
        if getattr(mtex, ext):
            if mapto == None:
...
def exportMesh(ob, fp):
    ...
    for uvtex in me.uv_textures:
        ...
        fp.write("    Data \n")
        for data in list(uvtex.data.values()):
            v = data.uv_raw
    ...
    for vcol in me.vertex_colors:
        ...
        if Optimize < 2:
            fp.write("    Data \n")
            for data in list(vcol.data.values()):
                fp.write("      cv ")
...
def exportArmature(ob, fp):
    ...
    bones = list(amt.edit_bones.values())
    fp.write("  Normal \n")
    
    print((obName, list(amt.edit_bones.keys()), list(amt.edit_bones.values()), bones, bpy.context.object))
    for b in bones:
...
def exportPose(ob, fp):
    ...
    for bg in list(ob.pose.bone_groups.values()):
        exportBoneGroup(fp, bg)
    for pb in list(ob.pose.bones.values()):
        exportPoseBone(fp, pb)
